...
const ListRoom = () => {
  ...
  return (
    <div>
      <Title title="Room Listings" align="left" font="Poppins" subTitle="View, edit, or manage all listed rooms. Keep the information up-to-date to provide the best experience for users." />
                ...
                <td className='py-3 px-4 text-red-500 border-t border-gray-300 text-sm text-center'>
                  <label htmlFor="" className='relative inline-flex items-center cursor-pointer text-gray-900 gap-3'>
                    <input type="checkbox" className='sr-only peer' checked={item.isAvailable} onChange={() => {
                      ...
                      updatedRooms[idx].isAvailable = !updatedRooms[idx].isAvailable;
                      setRooms(updatedRooms);
                    }} />
                    <div className='w-12 h-7 bg-slate-300 rounded-full peer peer-checked:bg-blue-600 transition-colors duration-200'></div>
                    <span className='absolute left-1 top-1 w-5 h-5 bg-white rounded-full transition transition-transform duartion-200 ease-in-out peer-checked:translate-x-5'></span>
                  </label>
                </td>
              </tr>
            ))}
