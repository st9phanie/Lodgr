...
const AddRoom = () => {
    ...
    return (
        <form className="sm:flex sm:justify-center sm:flex-col">
            <Title align="left" title="Add Room" font="Poppins" subTitle="Fill in the details carefully and accurately to enhance the user booking experience" />
            ...
            <div className="flex flex-col flex-wrap mt-1 text-gray-400 max-w-sm">
                {Object.keys(inputs.amenities).map((am, idx) => (
                    <div key={idx}>
                        <input type="checkbox" id={`amenities${idx + 1}`} checked={inputs.amenities[am]} onChange={() => setInputs({ ...inputs, amenities: { ...inputs.amenities, [am]: !inputs.amenities[am] } })} />
                        <label htmlFor={`amenities${idx + 1}`}> {am}</label>
                    </div>
                ))}
            </div>
