    ...
    return room && (
        <div className="py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32">
            <div className="flex flex-col md:flex-row items-start md:items-center gap-2">
                <h1 className="text-3xl md:text-4xl playfair">{room.hotel.name}
            ...
            <div className="flex flex-col lg:flex-row mt-6 gap-6">
                ...
                <div className="grid grid-cols-2 gap-4 lg:w-1/2 w-full">
                    {room?.images.length > 1 && room.images.map((image, key) => (
                        <img src={image} alt="Room image" onClick={() => setMainImage(image)} key={key} className={`w-full rounded-xl shadow-md object-cover cursor-pointer ${mainImage === image && 'outline-3 outline-orange-500'}`} />
                    ))}
                </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row md:justify-between mt-10">
                <div className="flex flex-col">
                    <h1 className="text-3xl md:text-4xl playfair">Experience Luxury Like Never Before</h1>
                    <div className="flex flex-wrap items-center mt-3 mb-6 gap-4">
                        {room.amenities.map((item, key) => (
                            <div key={key} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-100">
            ...
            <div className=" mt-15 flex flex-col items-start gap-4">
